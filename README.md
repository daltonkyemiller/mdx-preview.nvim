...
Run `mdx-preview components list <file-or-directory>` for the complete component context available to an artifact. It includes built-ins such as `Button`, `Callout`, `Checklist`, `Flow`, `Comparison`, `FileMap`, `FileTree`, `Decision`, `Timeline`, `TableOfContents`, `Metric`, `Architecture`, `Diagram`, `Diff`, and `CodeBlock`, followed by custom components resolved from the nearest `mdx-preview.config.mjs`.
